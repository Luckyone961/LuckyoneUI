...
function L1UI:Config()
	...
	L1UI.Options.args.blizzard.args.desc = ACH:Header(L["Blizzard improvements"], 1)
	...
	L1UI.Options.args.auras.args.desc = ACH:Header(L["Buffs and Debuffs"], 1)
	...
	L1UI.Options.args.layouts.args.desc = ACH:Header('ElvUI' .. ' ' .. L["Layouts"], 1)
	...
	L1UI.Options.args.tags.args.tag1 = ACH:Input(L["Displays the last part of the unit's name with class color"], nil, 3, nil, 'full', function() return '[luckyone:name:last-classcolor]' end)
	L1UI.Options.args.tags.args.tag1 = ACH:Input(L["Displays the last part of the unit's name with no color"], nil, 4, nil, 'full', function() return '[luckyone:name:last-nocolor]' end)
	L1UI.Options.args.tags.args.tag2 = ACH:Input(L["Displays the pet's name and includes (in Classic only) the full happiness status"], nil, 5, nil, 'full', function() return '[luckyone:pet:name-and-happiness]' end)
	L1UI.Options.args.tags.args.tag3 = ACH:Input(L["Displays the unit's classification (e.g 'Elite' and 'Rare') but without 'Affix'"], nil, 6, nil, 'full', function() return '[luckyone:classification]' end)
	L1UI.Options.args.tags.args.tag4 = ACH:Input(L["Displays the unit's level with difficultycolor if the player is not max level"], nil, 7, nil, 'full', function() return '[luckyone:level]' end)
	L1UI.Options.args.tags.args.tag5 = ACH:Input(L["Displays percentage health with 1 decimal below 100%, 2 decimals below 10% and hides decimals at 100%"], nil, 8, nil, 'full', function() return '[luckyone:health:percent]' end)
	L1UI.Options.args.tags.args.tag6 = ACH:Input(L["Displays percentage power with powercolor and hides power at 0"], nil, 9, nil, 'full', function() return '[luckyone:power:percent]' end)
	L1UI.Options.args.tags.args.tag7 = ACH:Input(L["Displays percentage mana without decimals"], nil, 10, nil, 'full', function() return '[luckyone:mana:percent]' end)
	L1UI.Options.args.tags.args.tag8 = ACH:Input(L["Displays the unit's Mana with manacolor (Role: Healer)"], nil, 11, nil, 'full', function() return '[luckyone:healermana:current]' end, nil, nil, E.Classic)
	L1UI.Options.args.tags.args.tag9 = ACH:Input(L["Displays the unit's Mana with manacolor in percent (Role: Healer)"], nil, 12, nil, 'full', function() return '[luckyone:healermana:percent]' end, nil, nil, E.Classic)
