...
function L1UI:Config()
	...
	L1UI.Options.args.setup.args.updateMain = ACH:Execute(L["Update Main Layout"], L["Update Main layout to LuckyoneUI version: "]..Private.Version, 2, function() Private:UpdateLayout('main') end, nil, true)
	L1UI.Options.args.setup.args.updateHealing = ACH:Execute(L["Update Healing Layout"], L["Update Healing layout to LuckyoneUI version: "]..Private.Version, 3, function() Private:UpdateLayout('healing') end, nil, true)
	L1UI.Options.args.setup.args.updateResolution = ACH:Execute(L["1080p > 1440p"], L["Scale the default 1080p to 1440p"]..'|cffC80000\n\nTHIS IS ONLY FOR MAIN AND SUPPORT LAYOUTS (NOT HEALING)|r', 4, function() Private:UpdateResolution() end, nil, true)
	...
	L1UI.Options.args.graphics.args.cosmetic.args.raidGraphicsOutlineMode = ACH:Range('Outline Mode (Raid)', 'Default: 1 | Suggestion: 2', 4, { min = 0, max = 2, step = 1 }, nil, function() return tonumber(GetCVar('raidGraphicsOutlineMode')) end, function(_, value) SetCVar('raidGraphicsOutlineMode', value) end, CheckRaid, not E.Retail)
	...
	-- L1UI.Options.args.layouts.args.thewarwithin.args.support = ACH:Execute(L["Support"], nil, 3, function() Private:Setup_Layout_TheWarWithin('support') E:StaticPopup_Show('L1UI_RL') end, nil, true)
	-- L1UI.Options.args.layouts.args.dragonflight = ACH:Group(L["Dragonflight Layouts"] .. ' (' .. 'v2.00' .. ') (' .. format('|cffC80000%s', L["Outdated"]) .. ')', nil, 3)
	L1UI.Options.args.layouts.args.dragonflight = ACH:Group(L["Dragonflight Layouts"] .. ' (v' .. tostring(Private.Version) .. ') (' .. format('|cff4beb2c%s', L["Current"]) .. ')', nil, 3)
	...
	L1UI.Options.args.layouts.args.dragonflight.args.support = ACH:Execute(L["Support"], nil, 3, function() Private:Setup_Layout_Dragonflight('support') E:StaticPopup_Show('L1UI_RL') end, nil, true)
	L1UI.Options.args.layouts.args.shadowlands = ACH:Group(L["Shadowlands Layouts"] .. ' (' .. 'v1.00' .. ') (' .. format('|cffC80000%s', L["Outdated"]) .. ')', nil, 4)
	...
	L1UI.Options.args.performance.args.elvui.args.chat = ACH:Execute(L["Clear Chat History"], nil, 1, function() Private:Cleanup_Cache('elvui', 'chat') end)
	L1UI.Options.args.performance.args.elvui.args.edit = ACH:Execute(L["Clear Editbox History"], nil, 2, function() Private:Cleanup_Cache('elvui', 'editbox') end)
	L1UI.Options.args.performance.args.details = ACH:Group('Details', nil, 3)
	L1UI.Options.args.performance.args.details.inline = true
	L1UI.Options.args.performance.args.details.args.cache = ACH:Execute(L["Clear Cache"], nil, 1, function() Private:Cleanup_Cache('details') end)
	L1UI.Options.args.performance.args.plater = ACH:Group('Plater', nil, 4)
	L1UI.Options.args.performance.args.plater.inline = true
	L1UI.Options.args.performance.args.plater.args.cache = ACH:Execute(L["Clear Cache"], nil, 1, function() Private:Cleanup_Cache('plater') end)
	...
	L1UI.Options.args.performance.args.rcLoot.args.cache = ACH:Execute(L["Clear Cache"], nil, 1, function() Private:Cleanup_Cache('rc') end)
	...
	L1UI.Options.args.performance.args.mrt.args.cache = ACH:Execute(L["Clear Cache"], nil, 1, function() Private:Cleanup_Cache('mrt') end)
	...
	L1UI.Options.args.tags.args.tag1 = ACH:Input(L["Displays the unit's classification (e.g 'Elite' and 'Rare') but without 'Affix'"], nil, 3, nil, 'full', function() return '[luckyone:classification]' end, nil, nil)
	L1UI.Options.args.tags.args.tag2 = ACH:Input(L["Displays the unit's level if the player is not max level"], nil, 4, nil, 'full', function() return '[luckyone:level]' end, nil, nil)
	L1UI.Options.args.tags.args.tag3 = ACH:Input(L["Displays percentage health with 1 decimal below 100%, 2 decimals below 10% and hides decimals at 100%"], nil, 5, nil, 'full', function() return '[luckyone:health:percent]' end, nil, nil)
	L1UI.Options.args.tags.args.tag4 = ACH:Input(L["Displays percentage power and hides power at 0"], nil, 6, nil, 'full', function() return '[luckyone:power:percent]' end, nil, nil)
	L1UI.Options.args.tags.args.tag5 = ACH:Input(L["Displays percentage mana without decimals"], nil, 7, nil, 'full', function() return '[luckyone:mana:percent]' end, nil, nil)
	L1UI.Options.args.tags.args.tag6 = ACH:Input(L["Displays the unit's Mana (Role: Healer)"], nil, 8, nil, 'full', function() return '[luckyone:healermana:current]' end, nil, nil, not E.Retail)
	L1UI.Options.args.tags.args.tag7 = ACH:Input(L["Displays the unit's Mana in percent (Role: Healer)"], nil, 9, nil, 'full', function() return '[luckyone:healermana:percent]' end, nil, nil, not E.Retail)
