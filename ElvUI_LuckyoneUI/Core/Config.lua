...
function L1UI:Config()
	...
	L1UI.Options.args.tags.args.tag1 = ACH:Input(L["Displays the last part of the unit's name with class color"], nil, 3, nil, 'full', function() return '[luckyone:name:last]' end)
	L1UI.Options.args.tags.args.tag2 = ACH:Input(L["Displays the pet's name and includes (in Classic only) the full happiness status"], nil, 4, nil, 'full', function() return '[luckyone:pet:name-and-happiness]' end)
	L1UI.Options.args.tags.args.tag3 = ACH:Input(L["Displays the unit's classification (e.g 'Elite' and 'Rare') but without 'Affix'"], nil, 5, nil, 'full', function() return '[luckyone:classification]' end)
	L1UI.Options.args.tags.args.tag4 = ACH:Input(L["Displays the unit's level with difficultycolor if the player is not max level"], nil, 6, nil, 'full', function() return '[luckyone:level]' end)
	...
	L1UI.Options.args.tags.args.tag6 = ACH:Input(L["Displays percentage power and hides power at 0"], nil, 8, nil, 'full', function() return '[luckyone:power:percent]' end)
