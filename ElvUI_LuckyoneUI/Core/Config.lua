...
function L1UI:Config()
	...
	L1UI.Options.args.tags.args.tag1 = ACH:Input(L["Displays the pet's name and includes (in Classic only) the full happiness status"], nil, 3, nil, 'full', function() return '[luckyone:pet:name-and-happiness]' end)
	L1UI.Options.args.tags.args.tag2 = ACH:Input(L["Displays the unit's classification (e.g 'Elite' and 'Rare') but without 'Affix'"], nil, 4, nil, 'full', function() return '[luckyone:classification]' end)
	L1UI.Options.args.tags.args.tag3 = ACH:Input(L["Displays the unit's level if the player is not max level"], nil, 5, nil, 'full', function() return '[luckyone:level]' end)
	L1UI.Options.args.tags.args.tag4 = ACH:Input(L["Displays percentage health with 1 decimal below 100%, 2 decimals below 10% and hides decimals at 100%"], nil, 6, nil, 'full', function() return '[luckyone:health:percent]' end)
	L1UI.Options.args.tags.args.tag5 = ACH:Input(L["Displays percentage power and hides power at 0"], nil, 7, nil, 'full', function() return '[luckyone:power:percent]' end)
	L1UI.Options.args.tags.args.tag6 = ACH:Input(L["Displays percentage mana without decimals"], nil, 8, nil, 'full', function() return '[luckyone:mana:percent]' end)
	L1UI.Options.args.tags.args.tag7 = ACH:Input(L["Displays the unit's Mana (Role: Healer)"], nil, 9, nil, 'full', function() return '[luckyone:healermana:current]' end, nil, nil, E.Classic)
	L1UI.Options.args.tags.args.tag8 = ACH:Input(L["Displays the unit's Mana in percent (Role: Healer)"], nil, 10, nil, 'full', function() return '[luckyone:healermana:percent]' end, nil, nil, E.Classic)
