...
local function BuildDamageMeterSection()
	...
	section.args.bars.args.backdropAlpha = ACH:Range(L["Backdrop Alpha"], L["Transparency of the bar backdrop behind each bar."], 5, { min = 0, max = 1, step = 0.01, isPercent = true })
	section.args.bars.args.backdropColorType = ACH:Select(L["Backdrop Color"], L["Follow the bar color or use a custom color."], 6, { CLASS = L["Class Colors"], CUSTOM = L["Custom"] })
	section.args.bars.args.backdropColor = ACH:Color(L["Custom Color"], nil, 7, nil, nil, function() local color = Private.Addon.db.profile.damageMeter.backdropColor return color.r, color.g, color.b end, function(_, r, g, b) local color = Private.Addon.db.profile.damageMeter.backdropColor color.r, color.g, color.b = r, g, b Private:DamageMeter_UpdateAll() end, function() return Private.Addon.db.profile.damageMeter.backdropColorType ~= 'CUSTOM' end)
	...
	section.args.text.args.valueSpacing = ACH:Range(L["Number Spacing"], L["Space between the primary and the secondary number."], 10, { min = 0, max = 20, step = 1 }, nil, nil, nil, function() return Private.Addon.db.profile.damageMeter.numberDisplay == 'MINIMAL' end)
