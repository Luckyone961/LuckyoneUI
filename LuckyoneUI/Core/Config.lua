...
local function BuildDamageMeterSection()
	...
	section.args.general.args.generalOptions.args.backdrop = ACH:Toggle(L["Frame Backdrop"], L["Show a transparent ElvUI backdrop behind the frame."], 6, nil, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.generalOptions.args.autoReset = ACH:Select(L["Auto Reset"], L["Reset all Damage Meter data when you enter a dungeon or raid."], 7, { NONE = _G.NONE, ASK = L["Ask"], AUTO = L["Automatic"] }, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.generalOptions.args.autoResetType = ACH:Select(L["Instances"], L["Which instance types trigger the reset."], 8, { BOTH = L["Dungeon"] .. ' + ' .. L["Raid"], PARTY = L["Dungeon"], RAID = L["Raid"] }, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end, function() return Private.Addon.db.profile.damageMeter.autoReset == 'NONE' end)
	...
	section.args.bars.args.colorOptions.args.backdropColorType = ACH:Select(L["Backdrop Color"], L["Follow the bar color or use a custom color."], 1, { CLASS = L["Class Color"], CUSTOM = L["Custom"] })
	section.args.bars.args.colorOptions.args.backdropColor = ACH:Color(L["Custom Color"], nil, 2, nil, nil, function() local color = Private.Addon.db.profile.damageMeter.backdropColor return color.r, color.g, color.b end, function(_, r, g, b) local color = Private.Addon.db.profile.damageMeter.backdropColor color.r, color.g, color.b = r, g, b Private:DamageMeter_UpdateAll() end, nil, function() return Private.Addon.db.profile.damageMeter.backdropColorType ~= 'CUSTOM' end)
	section.args.bars.args.colorOptions.args.backdropAlpha = ACH:Range(L["Backdrop Alpha"], L["Transparency of the bar backdrop behind each bar."], 3, { min = 0, max = 1, step = 0.01, isPercent = true })
	section.args.bars.args.colorOptions.args.classColors = ACH:Toggle(L["Class Color"], nil, 4)
	section.args.bars.args.colorOptions.args.othersColor = ACH:Color(L["Fallback Color"], L["Bar color for creatures and sources without a class."], 5, nil, nil, function() local color = Private.Addon.db.profile.damageMeter.othersColor return color.r, color.g, color.b end, function(_, r, g, b) local color = Private.Addon.db.profile.damageMeter.othersColor color.r, color.g, color.b = r, g, b Private:DamageMeter_UpdateAll() end)
	...
	section.args.headerOptions.args.sizeOptions.args.headerIconSize = ACH:Range(L["Icon Size"], L["Size of the settings icon in the header."], 2, { min = 8, max = 40, step = 1 })
	...
	section.args.headerOptions.args.positionOptions.args.headerIconXOffset = ACH:Range(L["Icon X Offset"], nil, 5, { min = -100, max = 100, step = 1 })
	section.args.headerOptions.args.positionOptions.args.headerIconYOffset = ACH:Range(L["Icon Y Offset"], nil, 6, { min = -100, max = 100, step = 1 })
