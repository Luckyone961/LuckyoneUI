...
local function BuildDamageMeterSection()
	...
	section.args.general.args.testMode = ACH:Toggle(L["Test Mode"], L["Show fake bars to preview settings. Resets on reload."], 2, nil, nil, nil, function() local DM = Private.Modules.DamageMeter return DM and DM.testMode end, function(_, value) local DM = Private.Modules.DamageMeter if DM then DM:SetTestMode(value) end end, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.windowCount = ACH:Range(L["Windows"], L["Number of session windows."], 3, { min = 1, max = 3, step = 1 }, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.orientation = ACH:Select(L["Orientation"], L["Place the session windows next to each other or stacked."], 4, { HORIZONTAL = L["Horizontal"], VERTICAL = L["Vertical"] }, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.backdrop = ACH:Toggle(L["Frame Backdrop"], L["Show a transparent ElvUI backdrop behind the frame."], 5, nil, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.visibility = ACH:Select(L["Visibility"], nil, 6, { SHOW = L["Always"], COMBAT = L["In Combat"], GROUP = L["In Group"] }, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.windowOneType = ACH:Select(L["Window 1"], nil, 7, function() local DM = Private.Modules.DamageMeter return DM and DM.TypeNames or {} end, nil, nil, function() return Private.Addon.db.profile.damageMeter.windows[1].meterType end, function(_, value) Private.Addon.db.profile.damageMeter.windows[1].meterType = value local DM = Private.Modules.DamageMeter local window = DM and DM.windows[1] if window then DM:SetWindowType(window, value) end end, function() return not Private.Addon.db.profile.damageMeter.enable end)
	...
	section.args.general.args.windowThreeType = ACH:Select(L["Window 3"], nil, 9, function() local DM = Private.Modules.DamageMeter return DM and DM.TypeNames or {} end, nil, nil, function() return Private.Addon.db.profile.damageMeter.windows[3].meterType end, function(_, value) Private.Addon.db.profile.damageMeter.windows[3].meterType = value local DM = Private.Modules.DamageMeter local window = DM and DM.windows[3] if window then DM:SetWindowType(window, value) end end, function() return not Private.Addon.db.profile.damageMeter.enable end, function() return Private.Addon.db.profile.damageMeter.windowCount < 3 end)
	section.args.general.args.autoReset = ACH:Select(L["Auto Reset"], L["Reset all Damage Meter data when you enter a dungeon or raid."], 10, { NONE = _G.NONE, ASK = L["Ask"], AUTO = L["Automatic"] }, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.autoResetType = ACH:Select(L["Instances"], L["Which instance types trigger the reset."], 11, { BOTH = L["Dungeon"] .. ' + ' .. L["Raid"], PARTY = L["Dungeon"], RAID = L["Raid"] }, nil, nil, nil, nil, function() local db = Private.Addon.db.profile.damageMeter return not db.enable or db.autoReset == 'NONE' end)
	...
	section.args.text.args.valueSpacing = ACH:Range(L["Number Spacing"], L["Space between the primary and the secondary number."], 14, { min = 0, max = 30, step = 1 }, nil, nil, nil, function() local db = Private.Addon.db.profile.damageMeter return not db.enable or db.numberDisplay == 'MINIMAL' end)
