...
local function NoOtherProfiles()
	local _, count = Private.Addon.db:GetProfiles()
	return count < 2
end
...
local function BuildProfilesSection()
	...
	section.args.profile.args.copy = ACH:Select(L["Copy From"], L["Copy the settings of another profile into the current one."], 3, OtherProfileValues, true, nil, function() end, function(_, value) Private.Addon.db:CopyProfile(value) StaticPopup_Show('LUCKYONE_RL') end, NoOtherProfiles)
	section.args.profile.args.delete = ACH:Select(_G.DELETE, L["Delete a profile, the current one cannot be deleted."], 4, OtherProfileValues, true, nil, function() end, function(_, value) Private.Addon.db:DeleteProfile(value) end, NoOtherProfiles)
