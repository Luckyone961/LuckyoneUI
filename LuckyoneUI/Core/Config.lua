...
local function BuildDamageMeterSection()
	...
	section.args.bars.args.backdropColorType = ACH:Select(L["Backdrop Color"], L["Follow the bar color or use a custom color."], 5, { CLASS = L["Class Colors"], CUSTOM = L["Custom"] })
	...
	section.args.text.args.nameColorType = ACH:Select(L["Name Color"], L["Color of the name text on each bar."], 5, { CUSTOM = L["Custom"], CLASS = L["Class Colors"] })
	...
	section.args.text.args.valueColorType = ACH:Select(L["Number Color"], L["Color of the number text on each bar."], 7, { CUSTOM = L["Custom"], CLASS = L["Class Colors"] })
	...
	section.args.text.args.bracketStyle = ACH:Select(L["Brackets"], L["Bracket style around the secondary number."], 13, { PARENTHESES = '( )', SQUARE = '[ ]', NONE = _G.NONE }, nil, nil, nil, nil, function() return Private.Addon.db.profile.damageMeter.numberDisplay == 'MINIMAL' end)
