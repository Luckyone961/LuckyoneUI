...
local function BuildDamageMeterSection()
	...
	section.args.general.args.secondWindow = ACH:Toggle(L["Second Window"], L["Show a second session window, both share the frame width."], 2, nil, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.backdrop = ACH:Toggle(L["Frame Backdrop"], L["Show a transparent ElvUI backdrop behind the frame."], 3, nil, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.visibility = ACH:Select(L["Visibility"], nil, 4, { SHOW = L["Always"], COMBAT = L["In Combat"], GROUP = L["In Group"] }, nil, nil, nil, nil, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.windowOneType = ACH:Select(L["Window 1"], nil, 5, function() local DM = Private.Modules.DamageMeter return DM and DM.TypeNames or {} end, nil, nil, function() return Private.Addon.db.profile.damageMeter.windowOne.meterType end, function(_, value) Private.Addon.db.profile.damageMeter.windowOne.meterType = value local DM = Private.Modules.DamageMeter local window = DM and DM.windows[1] if window then DM:SetWindowType(window, value) end end, function() return not Private.Addon.db.profile.damageMeter.enable end)
	section.args.general.args.windowTwoType = ACH:Select(L["Window 2"], nil, 6, function() local DM = Private.Modules.DamageMeter return DM and DM.TypeNames or {} end, nil, nil, function() return Private.Addon.db.profile.damageMeter.windowTwo.meterType end, function(_, value) Private.Addon.db.profile.damageMeter.windowTwo.meterType = value local DM = Private.Modules.DamageMeter local window = DM and DM.windows[2] if window then DM:SetWindowType(window, value) end end, function() local db = Private.Addon.db.profile.damageMeter return not db.enable or not db.secondWindow end)
	...
	section.args.size.args.innerSpacing = ACH:Range(L["Inner Spacing"], L["Space between the two session windows."], 4, { min = -20, max = 20, step = 1 })
	...
	section.args.text.args.nameXOffset = ACH:Range(L["Name X Offset"], nil, 5, { min = -100, max = 100, step = 1 })
	section.args.text.args.nameYOffset = ACH:Range(L["Name Y Offset"], nil, 6, { min = -100, max = 100, step = 1 })
	section.args.text.args.valueXOffset = ACH:Range(L["Number X Offset"], nil, 7, { min = -100, max = 100, step = 1 })
	section.args.text.args.valueYOffset = ACH:Range(L["Number Y Offset"], nil, 8, { min = -100, max = 100, step = 1 })
	section.args.text.args.bracketStyle = ACH:Select(L["Brackets"], L["Bracket style around the secondary number."], 9, { PARENTHESES = '( )', SQUARE = '[ ]', NONE = _G.NONE }, nil, nil, nil, nil, function() return Private.Addon.db.profile.damageMeter.numberDisplay == 'MINIMAL' end)
	section.args.text.args.valueSpacing = ACH:Range(L["Number Spacing"], L["Spaces between the primary and the secondary number."], 10, { min = 0, max = 10, step = 1 }, nil, nil, nil, function() return Private.Addon.db.profile.damageMeter.numberDisplay == 'MINIMAL' end)
	section.args.text.args.showRank = ACH:Toggle(L["Show Rank Numbers"], L["Show the rank number in front of each name."], 11)
	section.args.text.args.stripRealm = ACH:Toggle(L["Strip Realm Names"], L["Remove the realm name from cross realm players."], 12)
